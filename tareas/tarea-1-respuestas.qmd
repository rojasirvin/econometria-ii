...
a. [4 punto] ¿Cuál es la probabilidad de que $\bar{X}>58$?
   ...
   *La probabilidad es de 7.86%.*:
    
```{r}
1-pnorm((58-54)/(6/sqrt(50)))
``` 
...
*$\bar{y}\sim\mathcal{N}(0.3,0.21/N)$*
...
*Ees decir, un año más de educación se asocia con una reducción en el log de la razón de momios de 0.15.*
...
```
*El efecto de ser mujer es de un incremento de apenas 0.3 puntos en la probabilidad de ahorrar en el banco al estimar el promedio de los efectos marginales.*
...
```{r}
#| echo: true
(mmultilogit_rrr = exp(coef(multilogit)))
```
